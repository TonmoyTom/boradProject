...
<div class="card">
    ...
    <div style="margin-bottom: 10px;margin-top: 10px;" class="row">
        <div class="col-lg-8">
            <a class="btn btn-success" href="{{ route("backgrounds.create") }}">
              Add BackGround
            </a>
        </div>
    </div>
                  ...
                  <tbody>
                      @foreach ($answer as $item)
                        <tr>
                          ...
                          <td>
                            @if($item->status == 1)
                            <a href="javascript:void(0)" class="updatesanswersstatus" id="answers-{{$item->id }}"
                            ...
                            <td>
                            <a class="btn btn-xs btn-primary" href="{{url('admin/answers/view/'.Crypt::encrypt($item->id))}} ">
                                View
                            </a>
                            
                            <a class="btn btn-xs btn-info" href="{{url('admin/answers/edit/'.Crypt::encrypt($item->id))}} ">
                                Edit
                            </a>
                           
                            
                            <form action="{{url('admin/answers/delete/'.Crypt::encrypt($item->id))}}" method="POST" style="display: inline-block;">
                                @csrf
